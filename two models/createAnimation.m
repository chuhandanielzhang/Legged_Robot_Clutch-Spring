function createAnimation(t, q, fps, speed, phase_array)
    ...
    for i = 1:tskip:length(t)
        ...
        if current_phase == 1
            color_leg4 = [0.0 1.0 1.0];   
            color_leg8 = [0.0 0.0 1.0];
        elseif current_phase == 2
            color_leg4 = [0.8 0.8 0.8];  
            color_leg8 = [0.0 0.0 1.0];
        elseif current_phase == 3
            color_leg4 = [1.0 0.0 0.0];  
            color_leg8 = [1.0 0.0 0.0];
        end
        ...
        if i == 1
            ...
            plot0 = plot(ax, [-(X0+3)*cos(phi) (X0+3)*cos(phi)], [(X0+3)*sin(phi) -(X0+3)*sin(phi)], ...
            ...
            plot5 = plot(ax, [X0 XRW(1)], [Z0 ZRW(1)], 'Color', [0.00 0.00 1.00], 'LineWidth', 2.0);
            plot6 = plot(ax, [X0 XRW(8)], [Z0 ZRW(8)], 'Color', color_leg8, 'LineWidth', 2.0);
            plot7 = plot(X0, Z0, 'ko', 'MarkerSize', 10, 'MarkerFaceColor', 'k');

            % 加圆
            X_circle = X0 + r_circle * cos(theta_c);
            Z_circle = Z0 + r_circle * sin(theta_c);
            plot_circle = plot(X_circle, Z_circle, 'Color', [0.85 0.33 0.1], 'LineStyle', '-', 'LineWidth', 1.2);
            ...
            set(plot0, 'Xdata', [-(X0+3)*cos(phi) (X0+3)*cos(phi)], 'Ydata', [(X0+3)*sin(phi) -(X0+3)*sin(phi)]);
            ...
            set(plot5, 'Xdata', [X0 XRW(1)], 'Ydata', [Z0 ZRW(1)]);
            set(plot6, 'Xdata', [X0 XRW(8)], 'Ydata', [Z0 ZRW(8)], 'Color', color_leg8);
        ...
        title(ax, {
            '\bf Energy Recycling Rimless Wheel', ...
